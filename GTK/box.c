/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2020年07月18日 星期六 11时01分42秒
 ************************************************************************/
#include<gtk/gtk.h>
...
GtkWidget * makeTable()
{
    ...
    gtk_table_attach(GTK_TABLE(table),checkButtons,1,2,0,1,
                    GTK_FILL | GTK_EXPAND,
                    GTK_FILL | GTK_EXPAND,
                    10,0);
    gtk_widget_show(checkButtons);
    buttonBox = makeButtonBox();
    gtk_table_attach(GTK_TABLE(table),buttonBox,0,2,1,2,
...
GtkWidget * makecheckButtons()
{
    ...
    g_signal_connect(GTK_OBJECT(check),"clicked",GTK_SIGNAL_FUNC(click_button),"orange");
    gtk_box_pack_start(GTK_BOX(vbox),check,FALSE,FALSE,0);
    gtk_widget_show(check);
