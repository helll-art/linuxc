/*************************************************************************
	> File Name: button_box.c
	> Author: 
	> Mail: 
	> Created Time: 2020年07月18日 星期六 09时33分25秒
 ************************************************************************/
...
int main(int argc,char **argv)
{
    ...
    //生成一个垂直box容器，并将该容器放入到主窗口中

    box = gtk_vbox_new(FALSE,0);
    gtk_container_add(GTK_CONTAINER(window),box);

    //以下生成两个check按钮，将他们加入到box容器中，并显示出来
    ...
    //一下生成三个radio按钮，将他们加入到box容器中，并显示出来
    //注意：生成第一个radio按钮时group参数为NULL
    //而后每次在改组中创建一个radio按钮都要使用gtk_radio_button_group函数获取新的group值
